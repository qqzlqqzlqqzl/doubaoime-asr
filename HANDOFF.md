...
最新未发布补丁：2026-05-15 悬浮窗结果态空白区域已进一步收紧。用户红框指出顶部、左侧麦克风区、底部左侧和右侧空白过多。本轮把结果态宽度从 `456` 收到 `420`，新增 `ResultHeight := 118`，结果态显示时用独立高度，不再沿用录音态 `152`。结果文字框改为 `x18 y14 w354 h68`，按钮改为 `y88`，结果态隐藏大麦克风、齿轮和最小化入口，只保留关闭 `×`。安装版 DWM 截图 `release\test-reports\installed-float-result-compact-no-empty-dwm.png` 为 `842x238` 物理像素，较前一版 `914x306` 明显收紧。验证：`.venv\Scripts\python.exe -m pytest -q` 和 `-W error` 均为 `33 passed`；源码/安装版 `--float-self-test` 通过；`build-desktop-exe.ps1` 与 `test-desktop-exe.ps1` 通过；`startup-performance-float-compact-no-empty-1s.json` 为 `372 / 427 / 402 / 437 / 448ms`。本地 `%LOCALAPPDATA%\DoubaoASRHelper` 已覆盖新版且哈希与 `dist` 一致。

最新未发布补丁：2026-05-15 悬浮窗结果态文字空白已修复。用户截图显示结果态只剩边框和 `清空 / 复制 / 插入`，框内识别文字、麦克风、关闭和右上图标都不可见。排查时 `child_texts` 能读到完整文本，说明不是 ASR/赋值失败，而是 `ResultBgCtrl`/`TopPanelCtrl` 装饰 Static 在 no-activate 浮窗和 DPI 下盖住了内部文字控件。修复：`ResultTextCtrl` 从只读 `Edit` 改为带边框 `Text`，隐藏 `ResultBgCtrl` 和 `TopPanelCtrl` 装饰层，`ScrollResultToLatest()` 只触发重绘不再滚到末尾。复制/插入仍走完整 `LastText`。新增/更新 `tests/test_ahk_float_layout.py` 防回退。验证：`.venv\Scripts\python.exe -m pytest -q` 和 `-W error` 均为 `33 passed`；源码/安装版 `--float-self-test` 通过；安装版截图 `release\test-reports\installed-float-result-text-visible-dwm.png` 可见结果文字；`build-desktop-exe.ps1` 与 `test-desktop-exe.ps1` 通过；`startup-performance-float-text-visible-1s.json` 为 `562 / 668 / 506 / 513 / 342ms`。本地 `%LOCALAPPDATA%\DoubaoASRHelper` 已覆盖新版且哈希与 `dist` 一致。

最新未发布补丁：2026-05-15 `already_recording` 快速松手竞态已修复。用户看到的报错来自 15:44 日志链：`bridge_warmup_launch` 后立刻 `voice_start`，`EnsureRunning()` 又 `bridge_launch`，用户松手触发 `voice_stop_async` 时 bridge 还没有 active session，于是 `/stop` 返回 `no_active_session`；随后 `/start` 成功，留下 session=1 recording，后续 start 全部返回 `already_recording`。修复点：`ahk_client/src/bridge.ahk` 增加 `LaunchInProgress` 和统一 `LaunchBridge()`，在 `Run()` 之前就置位，`EnsureRunning()` 优先等待 warmup/launch 中的进程；`ahk_client/src/main.ahk` 增加 `StartInProgress`、`ReleasePending`、`CancelPending` 和 `ActiveMode`，启动中松手或取消只排队，`Start()` 返回后立即 stop/cancel；`already_recording` 会 cancel 后重试一次，`no_active_session` 不再作为错误弹给用户。新增 `tests/test_ahk_bridge_race.py` 防回退。验证：`.venv\Scripts\python.exe -m pytest -q` 和 `-W error` 均为 `32 passed`；`build-desktop-exe.ps1` 通过；`test-desktop-exe.ps1` 通过；安装版首屏按用户接受口径 1 秒内通过，`startup-performance-race-fix-1s.json` 为 `475 / 500 / 544 / 733 / 880ms`，`missing_texts=[]`。本地 `%LOCALAPPDATA%\DoubaoASRHelper\DoubaoASRHelper.exe` 已覆盖新版且哈希与 `dist` 一致。注意：本机未签名产物偶发被 Windows 应用控制策略拦截，最终复测已通过；正式分发仍建议代码签名。
