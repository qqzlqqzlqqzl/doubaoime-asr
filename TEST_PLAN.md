...
| U03 | 自由说录音 | 按一次默认 `Ctrl+Q` 开始，再按一次结束 | 录音开始/结束可靠，识别文字插入回原窗口 |
...
| U08 | 默认值复核 | 清空配置后首次启动设置页 | 默认值显示为 `右Ctrl`、`Ctrl+Q`、`左Ctrl+左Win`、`Z`、`Ctrl+D`、插入延迟 `0.3 秒`、剪贴板超时 `100ms`、发送延迟 `50ms` |
...
| T05 | 自由说闭环 | 按默认 `Ctrl+Q` 开始，说两段有停顿的话，再按一次结束 | 识别文本合并插入；开始/结束状态清楚，没有残留录音 |
| T06 | 取消录音闭环 | 录音中按 `Z` | 本次结果丢弃，不插入、不发送，悬浮窗收起 |
