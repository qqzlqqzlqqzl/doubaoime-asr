...
为了避免误触发，空闲状态启动录音时会要求按下的键和设置项精确匹配；比如设置为 Ctrl + D 时，Ctrl + Alt + D 不会启动录音。单独的左 Ctrl、Alt、Win、Shift 也不会作为启动热键保存；默认右 Ctrl 会保留。

如果快捷键或发送时序设置错了，点击“恢复默认”可以一键回到初始热键、延迟、剪贴板保护和启动项；凭据文件路径会保留，不会删除设备注册缓存。

六、悬浮窗
录音时会显示悬浮窗。按着说和按着说+自动发送在松开后会自动收起悬浮窗，避免遮挡后续输入；自由说结束后会保留识别结果，方便手动清空、复制或插入文字。

七、系统托盘和后台保活
点击主窗口右上角关闭按钮，或点击“隐藏窗口”，程序会收进 Windows 系统托盘并继续监听快捷键。

程序只允许当前 Windows 用户下运行一个实例。重复双击安装版、便携版或开机自启动脚本时，新进程会唤醒已有主窗口并退出，不会再创建第二个托盘图标。

托盘图标支持：
左键点击：显示主窗口。
右键菜单：显示主窗口、隐藏窗口、打开配置目录、退出。
