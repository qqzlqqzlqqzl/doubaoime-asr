...
实时录音不再把麦克风 PCM 原样直传。`asr_bridge.exe` 会对 16kHz mono int16 小帧做轻量软件处理：去 DC 偏移、噪声底估计、噪声门/下扩展、AGC 增益、峰值限幅，然后再编码送 ASR。处理器不缓存历史音频，只保留噪声底和平滑增益等少量状态，避免长时间运行时内存随录音次数增长。
...
悬浮窗中间会显示识别结果输入框，实时识别和最终文本都会出现在框内；松开按键后自动插入，不再要求用户点击「插入」。
设置界面按「按着说」「自由说」「按着说+自动发送」三种模式分块展示，下面统一放置豆包快捷键、插入延迟、剪贴板保护、开机自启动和高级延迟设置，和参考工具的操作路径保持一致。
热键输入框和「录制」按钮使用同高贴边的原生控件行，避免高 DPI 下出现边框断裂、错位或裁切。
