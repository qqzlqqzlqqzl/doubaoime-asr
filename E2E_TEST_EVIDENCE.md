...
模拟音频管道闭环备注：2026-05-19，新增 `asr_bridge --simulated-audio-processing-test` 和 `test-simulated-audio-asr.ps1`。测试用 Windows SAPI 自己生成中文语音，降级为低音量、带底噪和 DC 偏移的 WAV，再把同一份降级 PCM 分 raw degraded 与 `AudioProcessor` processed 两路送入 `transcribe_realtime`。这项不使用电脑扬声器、耳机、麦克风或物理热键，因此不能替代 T01/T11/T12 的真实闭环，但能自动证明音频处理后的实时 ASR 管道能跑通。源码证据：`release/test-reports/simulated-audio-processing-asr-source.json`，默认降级样本 raw 175 字、processed 179 字、关键词 `清晨/会议室/备用电池` 全命中、ASR 错误 0；低音量证据 `simulated-audio-processing-asr-source-lowgain.json`，raw 178 字、processed 187 字、关键词全命中、ASR 错误 0。打包版证据 `simulated-audio-processing-asr.json`：`frozen=true`、`ok=true`、raw 179 字、processed 179 字、关键词全命中、错误 0，处理后平均 RMS 从 232.030 提到 340.002；安装目录证据 `simulated-audio-processing-asr-installed.json`：`ok=true`、raw 180 字、processed 179 字、关键词全命中、错误 0。完整单测当前为 `.venv\Scripts\python.exe -m pytest -q`：`42 passed`。
...
- 模拟音频管道 ASR：源码报告 `release/test-reports/simulated-audio-processing-asr-source.json`，`ok=true`，默认降级样本 raw 175 字、processed 179 字，关键词 3 个全命中，ASR 错误 0；低音量报告 `simulated-audio-processing-asr-source-lowgain.json`，raw 178 字、processed 187 字，关键词 3 个全命中，ASR 错误 0；打包版报告 `simulated-audio-processing-asr.json` 和安装目录报告 `simulated-audio-processing-asr-installed.json` 均为 `ok=true`。
