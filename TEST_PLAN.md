...
项目冻结备注：2026-06-16，Windows 桌面语音输入助手方向已冻结，本文件保留为测试资产和未来重启前的 gate 清单，不再代表当前项目会继续按发布计划推进。冻结复盘见 [PROJECT_POSTMORTEM.md](PROJECT_POSTMORTEM.md)。

## 冻结后重启门槛

如果未来要做 V2 或类似项目，必须先通过下面这些前置验证，再恢复 UI、安装包、激活码或分发控制工作：

| 编号 | 重启前置项 | 最低要求 |
|------|------|------|
| R01 | 核心 ASR 质量 | 真人单句、长句、停顿、改口、小声和背景噪声下的识别文本可用；长句不能频繁提前截断 |
| R02 | 断句和最终文本 | 分词、断句、标点、上下文合并质量达到日常输入标准，用户不需要大段返工 |
| R03 | 真实输入闭环 | 至少完成 T01-T12 的真实目标窗口和物理热键验收，不能只用模拟音频或源码自测代替 |
| R04 | API 和合规边界 | 有稳定、可合法分发、可长期维护的 ASR API 或明确接受非官方协议随时失效的风险 |
| R05 | 产品窗口期 | Windows 官方豆包输入法仍不存在，且自研方案有足够时间窗口和差异化价值 |
| R06 | 分发可信度 | 若要给非开发用户，必须准备代码签名、干净 Win10/Win11 安装验证和卸载验证 |
