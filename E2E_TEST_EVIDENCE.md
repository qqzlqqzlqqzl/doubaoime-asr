...
参考图紧凑设置页复测备注：2026-05-14，最新 AHK 设置页不再使用旧 Python/Tk 大界面，继续复刻参考图分组和文案；高 DPI 下关闭 AHK 控件自动 DPI 坐标放大，并把窗口收敛到约 `413x666`。源码截图 `release/test-reports/source-ahk-settings-panel-fit.png` 和安装版截图 `release/test-reports/installed-final-settings-panel-fit.png` 均显示单页设置窗口；安装版悬浮窗证据 `release/test-reports/installed-final-compact-float-test.json` 为 `ok=true`、窗口 `457x133`，包含测试识别文本、`清空`、`复制`、`插入`。本轮 `build-desktop-exe.ps1` 通过，`.venv\Scripts\python.exe -m pytest` 为 `16 passed, 1 warning`；完整 `test-desktop-exe.ps1` 被 Windows 应用控制策略拦截在新打包的未签名 `dist/asr_bridge.exe`，`Unblock-File` 后仍被拦截，因此本轮安装包完整烟测不按通过计算。为避免破坏当前本机可用安装，已只覆盖 `%LOCALAPPDATA%\DoubaoASRHelper\DoubaoASRHelper.exe`，保留既有 `asr_bridge.exe`。
