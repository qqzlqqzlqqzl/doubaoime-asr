...
| U03 | 自由说录音 | 按一次默认 `鼠标侧键1` 开始，再按一次结束 | 录音开始/结束可靠，识别文字插入回原窗口 |
| U04 | 按住+发送 | 在聊天输入框里按住默认 `F9` 后松开 | 识别文字插入并按发送延迟触发 Enter |
| U05 | 取消录音 | 录音过程中按默认 `F12` | 本次输入取消，不插入也不自动发送 |
| U06 | 快捷键自定义 | 点击“录制”并输入新组合键后保存 | 配置保存成功，重启后仍生效 |
| U07 | 快捷键冲突 | 配置重复快捷键、裸字母键或 Windows 保留组合键，例如 `Alt + Tab` | 弹窗提示冲突或不可用风险，不能保存问题配置 |
| U08 | 默认值复核 | 清空配置后首次启动设置页 | 默认值显示为 `右Ctrl`、`鼠标侧键1`、`F9`、`F12`、`Ctrl+Alt+Shift+D`、插入延迟 `0.3 秒`、剪贴板超时 `100ms`、发送延迟 `50ms` |
...
| T04 | 自动发送闭环 | 在可控聊天窗口按住 `F9` 说话后松开 | 文本插入后按发送延迟触发 Enter；只发送一次 |
| T05 | 自由说闭环 | 按默认 `鼠标侧键1` 开始，说两段有停顿的话，再按一次结束 | 识别文本合并插入；开始/结束状态清楚，没有残留录音 |
| T06 | 取消录音闭环 | 录音中按 `F12` | 本次结果丢弃，不插入、不发送，悬浮窗收起 |
